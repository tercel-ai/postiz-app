...
export class LoadToolsService {
  ...
  async agent() {
    ...
    return new Agent({
      ...
      - When generating post content, ALWAYS ensure it is strictly within the maxLength returned by integrationSchema. Count characters carefully BEFORE presenting to the user, using the platform's specific counting rule:
        - For X (Twitter): use Twitter's weighted character counting (twitter-text). Latin letters, digits and basic punctuation each count as 1; CJK characters (Chinese hanzi, Japanese kana/kanji, Korean hangul), other non-Latin scripts and emoji each count as 2; URLs are always counted as 23 regardless of their actual length. As a practical guide: at most ~140 Chinese characters fit within the 280 free-tier limit (and ~2,000 within the 4,000 premium limit). Always leave a small safety margin (e.g. target 270 / 3,960) because mixed text and URLs are easy to mis-estimate.
        - For all other platforms (LinkedIn, Facebook, Instagram, Threads, Bluesky, TikTok, YouTube, Pinterest, Reddit, etc.): each Unicode code unit counts as 1. CJK characters count as 1, but most emoji count as 2 (surrogate pair) and ZWJ-combined emoji (e.g. family/profession emoji) can count as 4 or more.
      - If schedulePostTool returns a character limit error, automatically shorten the content and retry immediately WITHOUT asking the user again. When shortening for X specifically, remember that removing a Chinese character or emoji frees 2 weighted units, not 1.
