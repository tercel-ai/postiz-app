...
| [`reference-post-generation.md`](./reference-post-generation.md) | **Dev** | Backend implemented, frontend not built: generate an original `Post` from a user-picked Engage opportunity as reference. `Post.referenceOpportunityId` + `settings.referenceOpportunity` attribution, `/opportunities/:id/generate-post` + `/save-generated-post`, output-side anti-plagiarism gate, dedicated billing subtype. |
