...
9. ~~Make `engagePolicies` support a **per-day target schedule**~~ — **DONE (2026-07-17).** `engagePolicies[].dailyTargets: [{ date: "YYYY-MM-DD", target: n }]` overrides `targetRepliesPerDay` (now the *default* for un-listed days), keyed by concrete UTC date — no week abstraction, per the P2-8 decision. This finally makes the reference plan's "weekday 5 / weekend 3" rule expressible **and enforced**:
   ...
   - **Validation**: dates must be `YYYY-MM-DD`, inside `[startAt, endAt]`, non-repeating, integer ≥ 0.
     **Updated (2026-08-14):** an override failing any of those is now **dropped** (`_normalizeEngagePolicyDates`, logged; the day falls back to `targetRepliesPerDay`) rather than failing the whole paid generation — same repair-before-validate contract as content shrinking and platform backfill; the validator stays as a backstop. Root cause of the prod failures was the model computing dates from `startAt`/`endAt` and overshooting the final partial week (emitted `2026-08-23` for a range ending `2026-08-21`), so the generation payload now also carries `range.dates`, the enumerated closed list of valid days, and the prompt requires dates be copied verbatim from it.
...
10. ~~Add optional per-`platform` cadence/weight metadata~~ — **DONE (2026-07-17).** Admin-configurable via the `operation_plan.platform_cadence` Setting (`{ platform: { cadence, citationWeight, notes } }`, free-form prose since it's editorial guidance). Forwarded to the generator as the `platformPlaybook` input for the **requested** platforms only. A malformed Settings value degrades to the built-in defaults rather than breaking generation. Editable in aisee-manage → 配置管理 → 运营计划.
    **Updated (2026-08-04):** unconfigured platforms are no longer skipped from the playbook — that read as "no volume" to the model and was the root cause of plans generating content only for x/reddit while skipping the other requested platforms. Now every requested platform gets an entry (configured → built-in default → generic), the prompt carries an explicit PLATFORM COVERAGE requirement, and post-generation validation rejects a plan that produced no content for any requested platform.
10a. ~~Resolve a **subreddit** for every generated Reddit post~~ — **DONE (2026-07-24).** Reddit's submit hard-requires a target subreddit + title + type (unlike X, content alone can't publish), but generation produced content only → the posts were unpublishable. Fixed with a two-tier resolver (`reddit-target-resolver.ts`) run in `_generateAndBill` before `planPayload` is built, so all materialize paths read the result:
    - **Tier 1** reuses the project's enabled channel-scope `EngageTrackedAccount` (`platform='reddit'`) rows (round-robin, largest `audienceSize` first); **Tier 2** validates the LLM's new nullable `subreddit` field against Reddit's **public** API (`about.json` existence/public/self-allowed + `new.json` 48h activity, via the Engage loid/proxy WAF-bypass) and **writes back** a validated new subreddit as a channel-scope `EngageTrackedAccount` so the next plan is Tier 1.
    - A Reddit post with no valid target is **dropped** (empty content items pruned); `materializePlanPosts` folds the resolved target into `Post.settings.subreddit` (`type='self'`, `is_flair_required=false`). No DB migration (`planPayload` + `Post.settings` are `Json`). See [operation-plan-api.md](operation-plan-api.md) → the `subreddit` note for the accepted residual publish-failure risks (flair-forced subs, account-level gates, text-only).
