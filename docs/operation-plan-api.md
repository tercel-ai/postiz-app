...
> not sent yet. Committing them to the send queue (DRAFT → QUEUE) resolves each
...
> the project's active plan server-side, which is both simpler (a plan's post ids
> are re-materialized when the plan is re-run, so a client-held id list goes
> stale) and safer (a plan id supplied by the client cannot belong to another
> project). [`POST /posts/schedule`](./posts-api.md#post-postsschedule) remains
> the entry point for hand-picked post ids only.
>
...
> **Reading the materialized posts.** `GET /posts` and `GET /posts/list` filter on that column two ways: `?operationPlanId=<id>` for one plan's posts, and `?hasOperationPlan=true|false` to split plan-generated posts from everything created outside a plan. See [posts-list-module.md](./posts-list-module.md#operation-plan-filters).
...
> - **Write-back.** A Tier-2 subreddit that validates and isn't already monitored is persisted into the project's `EngageConfig` as a channel-scope `EngageTrackedAccount` (the API still speaks `channelId`; it is stored as the canonical lowercase `username`). The next plan then takes the cheaper Tier-1 path, and Engage scanning picks the community up too. Write-back is best-effort — a duplicate or failure never fails generation.
...
(posts, engage stats), or
[`POST /projects/:projectId/automation/publishing`](./automation-api.md#post-projectsprojectidautomationpublishing)
`{ commit: true }` to activate it.
...
> **Constraining WHAT time of day a platform publishes at** (not just de-clustering) is a separate, `extension_publish.*`-namespaced setting resolved at plan **activation** rather than generation — see [per-platform publish time window](automation-api.md#per-platform-publish-time-window). A **project** may narrow the admin window further through the Automation API.
...
`platform_cadence` shape — all fields optional free-form prose (it is editorial guidance, not a machine rule). Only the **requested** platforms are forwarded, and **every** requested platform gets an entry: the configured value when at least one field is filled, else the built-in default for that platform, else a generic rhythm. (Previously an unconfigured platform was skipped entirely — which read as "no volume" to the model and was the root cause of whole platforms coming back with zero generated posts.)

> **Platform coverage guarantee.** The generation prompt requires content for **every requested platform**, and the plan is validated after generation: a plan missing any requested platform is rejected (the row goes `FAILED` / `GENERATION_FAILED`, nothing is billed or materialized). Score-driven weighting only shifts volume between platforms — it can never drop one to zero.
