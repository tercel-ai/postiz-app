...
| `hasOperationPlan` | boolean | No | — (plan and plan-less posts alike) | `true` / `false` (query strings are parsed; anything other than `true` reads as `false`) | Presence filter on `operationPlanId` — see below |
...
### Operation-plan Filters

`operationPlanId` and `hasOperationPlan` both narrow on the same column, at
different granularity:

| Query | Resulting `where` | Returns |
|-------|-------------------|---------|
| neither | *(no clause)* | plan-generated **and** manual posts |
| `hasOperationPlan=true` | `operationPlanId: { not: null }` | only posts generated by some operation plan |
| `hasOperationPlan=false` | `operationPlanId: null` | only posts with no plan (manual, chat, engage) |
| `operationPlanId=plan_123` | `operationPlanId: 'plan_123'` | only posts from that one plan |
| both | `operationPlanId: 'plan_123'` | the explicit id **wins**; `hasOperationPlan` is ignored |

Omitting `hasOperationPlan` must leave the column unfiltered — that is what keeps
every pre-existing caller (which never sent the parameter) seeing both kinds of
post. The DTO's `@Transform` only runs for keys actually present in the query
string, so an absent parameter stays `undefined` rather than collapsing to
`false`; `has-operation-plan.dto.spec.ts` pins that.
...
GET /posts/list?hasOperationPlan=true      → every plan-generated post, any plan
GET /posts/list?hasOperationPlan=false     → only posts created outside a plan
```
...
- `hasOperationPlan` is the only filter that is **not** purely additive against its sibling: an explicit `operationPlanId` overrides it (see Operation-plan Filters above).
...
const where = {
  ...
  ...operationPlanWhere(query),   // operationPlanId / hasOperationPlan — see below
...
`operationPlanWhere()` is a module-level helper in `posts.repository.ts`, shared
by all three read paths so the clause cannot drift between them:

```typescript
function operationPlanWhere(query) {
  if (query.operationPlanId) return { operationPlanId: query.operationPlanId };
  if (query.hasOperationPlan === true) return { operationPlanId: { not: null } };
  if (query.hasOperationPlan === false) return { operationPlanId: null };
  return {};
}
```

> The sibling `locatePostInList` (`GET /posts/list/locate`) mirrors this exact
> `where` — including `source`, `projectId`, `operationPlanId` and
> `hasOperationPlan` — so a located page index stays
> consistent with the same filters applied to `/posts/list`. `GET /posts`
> (calendar) calls the same `operationPlanWhere()` helper.
...
- `hasOperationPlan` is parsed from the query string by `@Transform`, so `?hasOperationPlan=true` yields `true` and any other present value (including `false`) yields `false`; an absent parameter stays `undefined` and applies no filter
